...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, MDPIoU=False, feat_h=640, feat_w=640, eps=1e-7):
    ...
    if CIoU or DIoU or GIoU:
        ...
        return iou - d1 / mpdiou_hw_pow - d2 / mpdiou_hw_pow  # MPDIo
...
def ap_per_class(
    ...
    classes_to_log = [0, 56, 57, 58]
    filtered_indices = [i for i, c in enumerate(unique_classes) if c in classes_to_log]
    unique_classes = unique_classes[filtered_indices]
    nt = nt[filtered_indices]
    ...
    # Define the classes to log
    classes_to_log = [0, 56, 57, 58]

    # Filter results to only include specified classes
    filtered_indices = [i for i, c in enumerate(unique_classes) if c in classes_to_log]
    tp = tp[filtered_indices]
    fp = fp[filtered_indices]
    p = p[filtered_indices]
    r = r[filtered_indices]
    f1 = f1[filtered_indices]
    ap = ap[filtered_indices]
    unique_classes = unique_classes[filtered_indices]
    p_curve = p_curve[filtered_indices]
    r_curve = r_curve[filtered_indices]
    f1_curve = f1_curve[filtered_indices]
    prec_values = prec_values[filtered_indices]
    ...
    # print("Precision Curve (P Curve):", p_curve)
    # print("Recall Curve (R Curve):", r_curve)
    # print("F1 Curve (F1 Curve):", f1_curve)
    # print("X-axis values (X):", x)
    # print("Precision values at mAP@0.5 (Prec Values):", prec_values)
